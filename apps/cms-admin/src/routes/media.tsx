...
const ALL = "__all__";
const NONE = "__none__"; // "sin carpeta" (opción de destino al mover)
...
const selectStyle: CSSProperties = { padding: "0.4rem 0.5rem", borderRadius: 6, border: "1px solid #ccc", fontSize: "0.85rem", background: "#fff" };
const moveSelect: CSSProperties = { ...selectStyle, padding: "0.2rem 0.3rem", fontSize: "0.78rem", maxWidth: "10rem" };
...
export function MediaPage() {
  ...
  const folderNames = useMemo(() => {
    const set = new Set<string>(foldersQ.data?.map((f) => f.name) ?? []);
    for (const f of localFolders) set.add(f);
    return [...set].sort((a, b) => a.localeCompare(b));
  }, [foldersQ.data, localFolders]);

  const countFor = (name: string) => foldersQ.data?.find((f) => f.name === name)?.count ?? 0;
  ...
  const newFolder = () => {
    ...
    setFolder(name); // la activa: las subidas irán a esta carpeta
    setShowUpload(true);
  };
    ...
    <Page wide>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "1rem", marginBottom: "0.25rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.5rem" }}>
          ...
          <Button ghost onClick={newFolder}>Nueva carpeta</Button>
        </div>
      </div>
      ...
      {showUpload && (
        ...
        >
          ...
          <p style={{ color: "#999", fontSize: "0.8rem", marginTop: "0.9rem" }}>
            ...
            {folder !== ALL && <> Se subirán a la carpeta <strong>{folder}</strong>.</>}
          </p>
        </div>
      )}
      ...
      {/* Barra de herramientas: vista · carpeta · buscar */}
      <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "1rem", flexWrap: "wrap" }}>
        <div style={{ display: "flex" }} role="group" aria-label="Modo de vista">
        ...
        <label htmlFor="folder-filter" style={{ position: "absolute", left: "-9999px" }}>Filtrar por carpeta</label>
        <select id="folder-filter" value={folder} onChange={(e) => setFolder(e.target.value)} style={selectStyle}>
          <option value={ALL}>Todas las carpetas</option>
          {folderNames.map((name) => (
            <option key={name} value={name}>{name} ({countFor(name)})</option>
          ))}
        </select>
        ...
        <form onSubmit={(e) => { e.preventDefault(); setTerm(search.trim()); }} style={{ display: "flex", gap: "0.5rem", marginLeft: "auto" }}>
          ...
          <input id="media-search" placeholder="Buscar medios" value={search} onChange={(e) => setSearch(e.target.value)}
            style={{ ...selectStyle, width: "14rem", maxWidth: "50vw" }} />
      ...
      {media.data && items.length === 0 && (
        <Empty>{term || folder !== ALL ? "No hay medios que coincidan con el filtro." : "No hay medios todavía. Sube el primero con Añadir archivo."}</Empty>
      )}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", background: "#fff", border: "1px solid #dcdcde" }}>
          <thead>
            <tr>
              ...
              <th scope="col" style={{ ...thCell, width: 180 }}>Carpeta</th>
              <th scope="col" style={{ ...thCell, width: 90 }}>Tamaño</th>
              <th scope="col" style={{ ...thCell, width: 130 }}>Fecha</th>
              <th scope="col" style={{ ...thCell, width: 80 }} />
            </tr>
          </thead>
          <tbody>
            {items.map((asset) => (
              <tr key={asset.id}>
                <td style={tdCell}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
                    <img src={thumbUrl(asset)} alt={asset.alt ?? asset.filename} style={{ width: 44, height: 44, objectFit: "cover", borderRadius: 4, background: "#f6f7f7" }} />
                    <span style={{ wordBreak: "break-all" }}>{asset.filename}</span>
                  </div>
                </td>
                <td style={tdCell}>
